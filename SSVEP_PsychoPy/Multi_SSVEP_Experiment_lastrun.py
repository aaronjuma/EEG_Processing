...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\kapj_\\Documents\\GitHub\\EEG_Processing\\SSVEP_PsychoPy\\Multi_SSVEP_Experiment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    StartComponents = [textWelcome, keyWelcome]
    ...
    # keep track of which components have finished
    trialComponents = [hz_18, hz_16, hz_12, text, text_2, text_4]
    ...
    while continueRoutine and routineTimer.getTime() < 20.0:
        ...
        # if hz_18 is starting this frame...
        if hz_18.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            hz_18.frameNStart = frameN  # exact frame index
            hz_18.tStart = t  # local t and not account for scr refresh
            hz_18.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(hz_18, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'hz_18.started')
            # update status
            hz_18.status = STARTED
            hz_18.setAutoDraw(True)
        
        # if hz_18 is active this frame...
        if hz_18.status == STARTED:
            # update params
            hz_18.setOpacity((frameN%8) <=2, log=False)
        
        # if hz_18 is stopping this frame...
        if hz_18.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hz_18.tStartRefresh + 20-frameTolerance:
                # keep track of stop time/frame for later
                hz_18.tStop = t  # not accounting for scr refresh
                hz_18.tStopRefresh = tThisFlipGlobal  # on global time
                hz_18.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'hz_18.stopped')
                # update status
                hz_18.status = FINISHED
                hz_18.setAutoDraw(False)
        
        # *hz_16* updates
        
        # if hz_16 is starting this frame...
        if hz_16.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            hz_16.frameNStart = frameN  # exact frame index
            hz_16.tStart = t  # local t and not account for scr refresh
            hz_16.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(hz_16, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'hz_16.started')
            # update status
            hz_16.status = STARTED
            hz_16.setAutoDraw(True)
        
        # if hz_16 is active this frame...
        if hz_16.status == STARTED:
            # update params
            hz_16.setOpacity((frameN%9) <=3, log=False)
        
        # if hz_16 is stopping this frame...
        if hz_16.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hz_16.tStartRefresh + 20-frameTolerance:
                # keep track of stop time/frame for later
                hz_16.tStop = t  # not accounting for scr refresh
                hz_16.tStopRefresh = tThisFlipGlobal  # on global time
                hz_16.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'hz_16.stopped')
                # update status
                hz_16.status = FINISHED
                hz_16.setAutoDraw(False)
        
        # *hz_12* updates
        
        # if hz_12 is starting this frame...
        if hz_12.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            hz_12.frameNStart = frameN  # exact frame index
            hz_12.tStart = t  # local t and not account for scr refresh
            hz_12.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(hz_12, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'hz_12.started')
            # update status
            hz_12.status = STARTED
            hz_12.setAutoDraw(True)
        
        # if hz_12 is active this frame...
        if hz_12.status == STARTED:
            # update params
            hz_12.setOpacity((frameN%12) <=5, log=False)
        
        # if hz_12 is stopping this frame...
        if hz_12.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hz_12.tStartRefresh + 20-frameTolerance:
                # keep track of stop time/frame for later
                hz_12.tStop = t  # not accounting for scr refresh
                hz_12.tStopRefresh = tThisFlipGlobal  # on global time
                hz_12.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'hz_12.stopped')
                # update status
                hz_12.status = FINISHED
                hz_12.setAutoDraw(False)
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Adios.started', globalClock.getTime(format='float'))
    # create starting attributes for key_resp
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    AdiosComponents = [text_3, key_resp]
    for thisComponent in AdiosComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "Adios" ---
    for thisComponent in AdiosComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Adios.stopped', globalClock.getTime(format='float'))
    # check responses
